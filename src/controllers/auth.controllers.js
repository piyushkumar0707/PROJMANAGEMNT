...
const verifyEmail = asyncHandler(async (req, res) => { const { verificationToken } = req.params
...
if(!verificationToken){
    ...
    const user = await User.findOne({
        emailVerificationToken: hashedToken, emailVerificationExpiry: {$gt: Date.now()}
    })
...
return res
    .status(200)
    .json(
        new ApiResponse(200, null, "Email verification token resent successfully")
    );
});    
...
export { registerUser, loginUser, logoutUser , getCurrentUser, verifyEmail, resendEmailVerification, refreshAccessToken, forgotPasswordRequest, resetForgotPassword };